...
def count_carbon_atoms(smiles, atom="C"):
    with warnings.catch_warnings(record=True) as w:
        ...
        atoms = [atom for atom in mol.GetAtoms() if atom.GetSymbol() == atom]
        return len(atoms)

df_pcc = pd.read_csv('../drugs/pccs-pubchem/test.tsv', sep='\t')
df_pcc_tr = pd.read_csv('../drugs/pccs-pubchem/train.tsv', sep='\t')
...
df_pcc_test = df_pcc_test.filter(['Name', 'SMILES', 'No. C', 'No. N', 'No. O', 'No. H', 'is_test'])
df_pcc_tr = df_pcc_tr.filter(['Name', 'SMILES', 'No. C', 'No. N', 'No. O', 'No. H', 'is_test'])
